...
from flaskweb.thirdparty.utils.features import extract_features, bulk_extract_features
from flaskweb.thirdparty.utils.strategies import select_weighted_random, select_most_likely
from flaskweb.thirdparty.utils.utilities import flatten_coords, unflatten_coords
...
class MCTSPlayerMixin(object):
    ...
    def suggest_move_mcts(self, position: go.Position, fixed_depth=True)->np.ndarray:
        ...
        # logger.info(" num of values %s " % len(values))
        if fixed_depth:
    ...
    async def tree_search(self, position: go.Position)->float:
        ...
        with await self.sem:
            ...
            # logger.debug('Current running threads : {}'.format(self.running_simulation_num))
            self.running_simulation_num -= 1
    ...
    async def prediction_worker(self):
        ...
        try:
            while self.running_simulation_num > 0 or margin > 0:
                # logger.debug("q size %s margin %s " % (q.qsize(), margin))
                if q.empty():
                    if margin > 0:
                        margin -= 1
            ...
            traceback.print_exc()
            logger.error(str(e))
        logger.debug("end prediction_worker")
