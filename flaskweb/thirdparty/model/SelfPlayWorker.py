...
from flaskweb.thirdparty.utils.strategies import (simulate_game_mcts,
                                                  simulate_many_games, get_winrate,
                                                  extract_moves, GreedyPolicyPlayerMixin,
                                                  POLICY_CUTOFF_DEPTH)
from flaskweb.thirdparty.model.APV_MCTS_C import MCTSPlayerMixin
...
class SelfPlayWorker(object):
    ...
    def play(self):
        ...
        with timer(f"Self-Play Simulation Game #{1}"):
            final_position, agent_resigned, false_positive = simulate_game_mcts(self.net, self.position,
                                                                                playouts=self.playouts,
                                                                                resignThreshold=self.resign_threshold,
                                                                                no_resign=self.no_resign_this_game)
...
class RivalWorker(object):
    ...
    def __init__(self, net, rl_flags):
        ...
        self.policy = MCTSPlayerMixin(net, self.playouts)

    def getMove(self):
        def game_end_condition() -> bool:
            if self.position.n >= 2:
                return self.position.n < POLICY_CUTOFF_DEPTH and not (
                self.position.recent[-2].move is None and self.position.recent[-1].move is None)
            else:
                return True

        condition = game_end_condition()
        if condition:
            move = self.policy.suggest_move(self.position)
            self.position.play_move(move, mutate=True)
            return False, move[0], move[1]
        else:
